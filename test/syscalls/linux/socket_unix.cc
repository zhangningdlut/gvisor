...
TEST_P(UnixSocketPairTest, ShutdownRead) {
  ...
  const std::string data = "abc";
  EXPECT_THAT(WriteFd(sockets->second_fd(), data.c_str(), 3),
  ...
  char buf[20];
  EXPECT_THAT(ReadFd(sockets->second_fd(), buf, 3),
              SyscallSucceedsWithValue(3));
  EXPECT_EQ(data, absl::string_view(buf, 3));
}
...
TEST_P(UnixSocketPairTest, ShutdownWrite) {
  ...
  const std::string data = "abc";
  EXPECT_THAT(WriteFd(sockets->first_fd(), data.c_str(), 3),
              SyscallFailsWithErrno(EPIPE));
  ...
  char buf[20];
  ASSERT_THAT(WriteFd(sockets->second_fd(), data.c_str(), 3),
              SyscallSucceedsWithValue(3));

  // ... and the socket can still read.
  EXPECT_THAT(ReadFd(sockets->first_fd(), buf, 3), SyscallSucceedsWithValue(3));
  EXPECT_EQ(data, absl::string_view(buf, 3));
}
